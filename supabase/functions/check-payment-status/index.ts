...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const paymentId = String(body?.payment_id ?? body?.payment ?? "");
    const fallbackOrderId = String(body?.order_id ?? body?.order ?? "");
    ...
    if (!paymentResponse.ok) {
      return jsonResponse(
        {
          error: "Unable to fetch Mercado Pago payment",
          details: paymentData,
        },
        500,
      );
    ...
    const orderId = String(paymentData?.external_reference ?? fallbackOrderId);
